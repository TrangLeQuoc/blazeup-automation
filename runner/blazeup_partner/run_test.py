#!/usr/bin/env python3
"""Entry point: run Partner domain tests only.

Usage:
    python -m runner.blazeup_partner.run_test
    python -m runner.blazeup_partner.run_test --execute 10101 10102
    python -m runner.blazeup_partner.run_test --mode smoke
...
from runner.blazeup_partner.registry import TC_REGISTRY  # noqa: E402

# ---------------------------------------------------------------------------
# Partner domain defaults — edit these directly
# ---------------------------------------------------------------------------

# TC IDs to run when no --execute / --mode flag is passed.
# (empty until BlazeUp Partner test cases are added to the registry)
DEFAULT_EXECUTE_IDS: list[str] = []

# TC IDs to always skip
DEFAULT_SKIP_IDS: list[str] = []

# Export results to Excel after every run.  blazeup_admin currently owns the
# Partner Platform test plan, so Excel reporting is disabled here.  Flip to True
...
# ---------------------------------------------------------------------------
# Re-use the shared CLI from runner/run_test.py but scoped to Partner registry
# ---------------------------------------------------------------------------

if __name__ == "__main__":
    # Scope the shared runner to Partner-only TCs.  runner.run_test and
