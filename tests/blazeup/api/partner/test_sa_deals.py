...
async def test_partner_api_deal_registration_pipeline_004(
    ...
    ``conflictStatus`` becomes "flagged" and ``conflictingDealIds`` includes the
    ...
    async with async_step("[2/3] Partner 2 registers the SAME prospect → conflict flagged"):
        ...
        conflicting_ids = deal_b.data.get("conflictingDealIds") or []
        assert deal_a.deal_id in conflicting_ids, (
            f"conflictingDealIds must include the first registered deal, got {conflicting_ids}"
        )
        logger.info(
            "CHECK conflict → OK (conflictStatus='flagged', conflictingDealIds={})", conflicting_ids
        )
