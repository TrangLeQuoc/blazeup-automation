...
⚠️ SCAFFOLD — NOT WIRED YET. The endpoint paths below are placeholders; calling any
method raises NotImplementedError until `_DEALS` is set to a real path. Partner deals
ARE tested today via the SA-minted partner session — see `tests/blazeup/api/partner/
test_sa_pipeline_management.py` + `utils/partner_portal.py`. Fill this in only when a
direct partner-login client (no SA mint) is actually needed.

Mirrors ``api_clients/blazeup/admin/auth_client.py``:
    api_clients/base_client.py            → BaseClient (get/post/put/patch/delete)
    api_clients/blazeup/partner/<this>    → endpoints + pydantic response schemas
    tests/blazeup/api/            → scenarios

Usage in a test::

    from api_clients.blazeup.partner.deal_registration_client import DealRegistrationClient
