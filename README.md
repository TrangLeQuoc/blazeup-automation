...
| `selftest.yml` | **automatic** on push / PR touching `runner/`, `utils/`, `selftests/`, `tests/`, `docs/blazeup/` | Tests the framework's own logic (TC registry, PASS/FAIL/BLOCKED classification), the `be_gap` traceability chain (code marker ↔ test-case docs ↔ Bug_Tracker) and the layer boundaries (no raw Playwright selectors or API paths in tests; every client method pinned to its endpoint) — no secrets, no staging, no browser |
