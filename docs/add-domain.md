...
1. `runner/<domain>/run_test.py` — copy từ `runner/blazeup_admin/run_test.py`,
   đổi `os.environ.setdefault("BLAZEUP_DOMAIN", "<domain>")` và các đường dẫn.
...
```bash
python -m runner.<domain>.run_test --mode smoke
```
...
## Checklist nhanh
- [ ] `runner/<domain>/` (run_test.py + registry.py)
- [ ] `tests/<domain>/`
